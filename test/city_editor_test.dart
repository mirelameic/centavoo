...
void main() {
  ...
  testWidgets('adding a city that only differs in casing from an existing one is ignored', (tester) async {
    final db = AppDatabase.forTesting(NativeDatabase.memory());
    final tripId = await createTrip(db, name: 'Japan');
    ...
    final trip = await (db.select(db.tripsTable)..where((t) => t.id.equals(tripId))).getSingle();
    expect(trip.cityListJson, isNull);
    await db.close();
  });
  ...
  testWidgets('removing a city with assigned days shows a warning and clears those days on confirm', (tester) async {
    final db = AppDatabase.forTesting(NativeDatabase.memory());
    final tripId = await createTrip(db, name: 'Japan');
    ...
    expect(find.textContaining('Remover mesmo assim'), findsOneWidget);

    await tester.tap(find.text('Excluir'));
    await tester.pumpAndSettle();

    final trip = await (db.select(db.tripsTable)..where((t) => t.id.equals(tripId))).getSingle();
    expect(jsonDecode(trip.cityListJson!), isEmpty);
