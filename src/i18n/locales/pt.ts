export const pt = {
  'app.title': 'Centavoo',
  'loading': 'Preparando seus dados…',
  'error': 'Erro ao carregar os dados.',

  'trips.title': 'Minhas viagens',
  'trips.new': 'Nova viagem',
  'trips.empty': 'Nenhuma viagem ainda.',
  'trips.createFirst': 'Criar a primeira',
  'trips.netSpend': 'Gasto líquido',

  'form.name': 'Nome',
  'form.namePlaceholder': 'ex. Europa 2025',
  'form.destination': 'Destino',
  'form.destPlaceholder': 'ex. Espanha · Grécia',
  'form.dates': 'Período',
  'form.datesPlaceholder': 'início – fim',
  'common.cancel': 'Cancelar',
  'common.delete': 'Excluir',
  'common.create': 'Criar',
  'common.back': 'Voltar',

  'nav.trips': 'Viagens',
  'kpi.net': 'Líquido',
  'kpi.gross': 'Bruto',
  'kpi.refunds': 'Reembolsos',
  'kpi.before': 'Antes',
  'kpi.during': 'Durante',
  'kpi.avgPerDay': 'Média/dia',

  'tab.summary': 'Resumo',
  'tab.top': 'Ranking',
  'tab.transactions': 'Transações',

  'chart.before': 'Antes',
  'chart.during': 'Durante',
  'chart.noDated': 'Sem gastos com data neste período.',
  'chart.noCity': 'Nenhuma transação com cidade ainda.',
  'chart.noTop': 'Nenhuma transação ainda.',

  'city.perDay': 'Cidades por dia',
  'city.placeholder': 'cidade',
  'city.list': 'Cidades da viagem',
  'city.addCity': 'Adicionar cidade',
  'city.removeCity': 'Remover',
  'city.blockCityLabel': 'Cidade',
  'city.blockRangeLabel': 'Período',
  'city.addBlock': 'Adicionar período',
  'city.noBlocks': 'Nenhum período definido ainda.',
  'city.daysN': 'dia(s)',
  'city.unassignedN': 'dia(s) sem cidade',
  'city.removeBlockConfirm': 'Remover este período? Os dias ficam sem cidade.',
  'city.removeUsedWarning': 'Remover mesmo assim vai deixar esses dias sem cidade. Continuar?',
  'tx.deleteSelected': 'Excluir selecionadas',
  'tx.selectedN': 'selecionadas',
  'tx.deleteSelectedConfirm': 'Excluir as transações selecionadas?',
  'tx.searchPlaceholder': 'Buscar por descrição…',
  'tx.filterDate': 'Data',
  'tx.filterDatePlaceholder': 'qualquer data',
  'tx.dateModeDay': 'Dia',
  'tx.dateModeRange': 'Período',
  'tx.filterCategory': 'Categoria',
  'tx.filterCategoryPlaceholder': 'todas as categorias',
  'tx.filterCity': 'Cidade',
  'tx.filterCityPlaceholder': 'todas as cidades',
  'tx.filterPeriod': 'Período',
  'tx.periodAll': 'Todos',
  'tx.filterResultsN': 'resultado(s)',
  'tx.clearFilters': 'Limpar filtros',
  'tx.select': 'Selecionar',
  'tx.cancelSelect': 'Cancelar',
  'tx.selectAll': 'Selecionar todos',
  'tx.clearSelection': 'Limpar seleção',
  'tx.deleteOne': 'Excluir',
  'tx.sortBy': 'Ordenar por',
  'tx.sortDefault': 'Padrão',

  'table.date': 'Data',
  'table.description': 'Descrição',
  'table.category': 'Categoria',
  'table.city': 'Cidade',
  'table.period': 'Período',
  'table.amount': 'Valor',
  'table.full': 'integral',

  'period.before': 'Antes',
  'period.during': 'Durante',
  'trip.notFound': 'Viagem não encontrada.',

  'common.save': 'Salvar',
  'common.edit': 'Editar',
  'tx.new': 'Nova transação',
  'tx.edit': 'Editar transação',
  'tx.deleteConfirm': 'Excluir esta transação?',
  'field.type': 'Tipo',
  'field.split': 'Dividir por',
  'type.expense': 'Gasto',
  'type.refund': 'Reembolso',
  'type.iof': 'Reembolso de IOF',
  'menu.categories': 'Categorias',
  'menu.export': 'Exportar dados',
  'menu.import': 'Importar dados',
  'cat.title': 'Categorias',
  'cat.new': 'Nova categoria',
  'cat.color': 'Cor',
  'cat.icon': 'Ícone',
  'cat.empty': 'Nenhuma categoria.',
  'cat.deleteConfirm': 'Excluir categoria? As transações dela ficam sem categoria.',
  'backup.exportedOk': 'Backup exportado.',
  'backup.importedOk': 'Backup importado com sucesso.',
  'backup.importError': 'Arquivo de backup inválido.',

  'tab.time': 'Tempo',
  'tab.cities': 'Cidades',
  'tab.cats': 'Categorias',
  'sec.cumulative': 'Gasto acumulado',
  'sec.byDay': 'Por dia',
  'sec.weekday': 'Por dia da semana',
  'sec.byCity': 'Por cidade',
  'sec.cityTable': 'Resumo por cidade',
  'sec.catTable': 'Resumo por categoria',
  'sec.beforeDuring': 'Antes × Durante',
  'sec.topBefore': 'Maiores gastos · antes',
  'sec.topDuring': 'Maiores gastos · durante',
  'trip.edit': 'Editar viagem',
  'trip.delete': 'Excluir viagem',
  'trip.deleteConfirm':
    'Excluir esta viagem permanentemente? Todas as transações e categorias dela serão apagadas. Não dá pra desfazer.',
  'city.filter': 'Filtrar por categoria',
  'city.filterPlaceholder': 'todas as categorias',
  'sec.split': 'Você dividiu × sua parte',
  'col.days': 'Dias',
  'col.total': 'Total',
  'col.avgDay': 'Média/dia',
  'col.topCat': 'Top categoria',
  'col.count': 'Nº',
  'col.entry': 'transação',
  'col.entries': 'transações',
  'col.avgTicket': 'Ticket médio',
  'split.integral': 'Valor integral',
  'split.share': 'Sua parte',
  'split.savings': 'Você economizou',

  'import.button': 'Importar',
  'import.title': 'Importar transações',
  'import.intro':
    'Cole os dados da fatura (copiados do site do banco ou de uma planilha) ou envie um arquivo .csv. Nada é importado antes de você conferir e confirmar.',
  'import.pasteLabel': 'Cole os dados aqui',
  'import.pastePlaceholder': '12/03/2026\tUber\t45,90\n13/03/2026\tPadaria\t12,00',
  'import.uploadButton': 'Ou escolher um arquivo (.csv)',
  'import.delimiter': 'Separador',
  'import.delimiterAuto': 'Detectar automaticamente',
  'import.delimiterComma': 'Vírgula ( , )',
  'import.delimiterSemicolon': 'Ponto e vírgula ( ; )',
  'import.delimiterTab': 'Tab',
  'import.continue': 'Continuar',
  'import.noRows': 'Não encontrei nenhuma linha de dados. Verifique o texto colado ou o arquivo.',
  'import.back': 'Voltar',
  'import.hasHeader': 'A primeira linha é um cabeçalho',
  'import.invertSign': 'Inverter sinal dos valores',
  'import.mapHint': 'Diga o que cada coluna significa:',
  'import.columnN': 'Coluna {n}',
  'import.colDate': 'Data',
  'import.colDescription': 'Descrição',
  'import.colAmount': 'Valor',
  'import.colIgnore': 'Ignorar',
  'import.status': 'Status',
  'import.rowOk': 'ok',
  'import.errAmount': 'valor inválido',
  'import.errDescription': 'sem descrição',
  'import.errDate': 'data inválida',
  'import.rowsReady': 'linhas prontas para importar',
  'import.rowsSkipped': 'com problema (não serão importadas)',
  'import.confirmButton': 'Importar',
  'import.successSuffix': 'transações importadas.',
  'import.error': 'Não foi possível importar. Tente novamente.',
};
