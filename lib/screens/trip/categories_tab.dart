  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          sectionHeader(context, l10n.secCatTable, first: true),
          for (final c in stats.categoryTable)
            summaryRow(
              context,
              leading: categoryChip(context, color: hexColor(c.color), name: c.name, icon: c.icon),
              metaLines: [
                Text('${_fmtNum(c.pct)}% · ${c.count} ${c.count == 1 ? l10n.colEntry : l10n.colEntries}'),
                Text('${l10n.colAvgTicket}: ${money(c.avgTicket, currency: widget.currency)}'),
              ],
              amount: money(c.total, currency: widget.currency),
            ),
          sectionHeader(context, l10n.secBeforeDuring),
          _BeforeDuringChart(data: stats.beforeDuringData, hidden: _hiddenBdSeries, currency: widget.currency),
          toggleLegend(
            context,
            series: [
              ToggleSeries(name: 'before', color: _beforeColor, label: l10n.periodBefore),
              ToggleSeries(name: 'during', color: _duringColor, label: l10n.periodDuring),
            ],
