  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text('Filtrar por categoria', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500)),
          ...
          if (cityBd.byCity.isEmpty)
            Text('Nenhuma transação com cidade ainda.', style: TextStyle(color: hintColor))
          else ...[
            sectionHeader(context, 'Por cidade', first: true),
            Wrap(
            ...
            sectionHeader(context, 'Resumo por cidade'),
            for (final c in cityBd.cityTable)
              summaryRow(
                context,
                leading: Text(c.city),
                metaLines: [
                  Text('${c.days} dia(s) · Média/dia: ${money(c.avgPerDay, currency: widget.currency)}'),
                  Row(
          ...
          if (_tripDays.isNotEmpty) ...[
            sectionHeader(context, 'Cidades por dia'),
            CityEditor(
              db: widget.db,
