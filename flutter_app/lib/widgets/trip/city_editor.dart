...
class _CityEditorState extends State<CityEditor> {
  ...
  Future<void> _removeBlock(CityBlock b) async {
    await confirmDelete(context, 'Remover este período? Os dias ficam sem cidade.', () async {
  ...
  Future<void> _removeCity(String city) async {
    ...
    if (daysUsed.isNotEmpty) {
      await confirmDelete(
        context,
        '$city — ${daysUsed.length} dia(s). Remover mesmo assim vai deixar esses dias sem cidade. Continuar?',
        doRemove,
      );
    } else {
      await doRemove();
  ...
  Future<void> _confirmAddCity() async {
    ...
    if (v.isNotEmpty && !_listValue.contains(v)) {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        const Text('Cidades da viagem', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500)),
          ...
          children: [
            for (final city in listValue)
              Chip(
                ...
                deleteButtonTooltipMessage: 'Remover $city',
              ),
            if (_addingCity)
              Row(
                ...
                children: [
                  SizedBox(
                    ...
                    child: TextField(
                      ...
                      autofocus: true,
                      decoration: const InputDecoration(hintText: 'cidade', isDense: true),
        ...
        Wrap(
          ...
          children: [
            ...
            ElevatedButton(
              ...
              child: Text(_editing != null ? 'Salvar' : 'Adicionar período'),
            ),
            if (_editing != null) TextButton(onPressed: _resetForm, child: const Text('Cancelar')),
          ],
        ),
        const SizedBox(height: 12),
        if (blocks.isEmpty)
          Text('Nenhum período definido ainda.', style: TextStyle(fontSize: 13, color: hintColor))
        else
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            '${b.start == b.end ? fmtDate(b.start) : '${fmtDate(b.start)} – ${fmtDate(b.end)}'} · ${b.days.length} dia(s)',
        ...
        if (unassigned > 0)
          Padding(
            ...
            child: Text('$unassigned dia(s) sem cidade', style: TextStyle(fontSize: 12, color: hintColor)),
          ),
      ],
    );
  }
