...
String money(double n, {String currency = 'BRL', String locale = 'pt_BR'}) {
  final symbol = currency == 'BRL' ? 'R\$' : currency;
  final isNegative = n < 0;
  final valueStr = NumberFormat.currency(locale: locale, symbol: '', decimalDigits: 2).format(n.abs()).trim();
  return '${isNegative ? '-' : ''}$symbol $valueStr';
}

String fmtDate(String? d, {String locale = 'pt_BR'}) {
  if (d == null) return '—';
  final date = DateTime.parse(d);
  return DateFormat("dd 'de' MMM", locale).format(date);
}
